...
ServerError.prototype.handle = function (msg, status) {
	...
	logger.error('(url:' + this._resource.rawRequest.url + ')', (msg || ''), status);
	...
	if (config.error && config.error[status] && !this._resource.error) {
		...
		// we do not pass sub directory name to pre-defined error method
		this._resource.parsedUrl.subdir = '';
		// we set error flag to prevent possible infinite loop of error on error controller
		this._resource.setError(error, msg, status);
		// execute error controller
		if (this._request) {
			// we already have a request object
			return controller.handle(this._resource, this._request);
		}
		// make sure parsedUrl.notFound is not present,
		// so we don't come back here without executing re-defined error controller
	...
	if (this._resource.error) {
		logger.error(
			'pre-defined error controller failed to execute:',
			'(url:' + this._resource.rawRequest.url + ')',
			'(request-id:' + this._resource.rawRequest.uniqueId + ')',
	...
	if (!this._response) {
		...
		this._response = response.create(this._resource, this._request);
	}
