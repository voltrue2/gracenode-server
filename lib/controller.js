...
module.exports.exec = function (resource) {
	...
	if (resource.parsedUrl.notFound) {
		return errorHandler(resource, new Request(resource.rawRequest, resource.rawResponse, resource.parsedUrl));
	}

	var request = new Request(resource.rawRequest, resource.rawResponse, resource.parsedUrl);
...
module.exports.handle = function (resource, requestObj) {
	...
	var path = gracenode.getRootPath() + config.controllerPath + resource.parsedUrl.controller + '/' + resource.parsedUrl.method + resource.parsedUrl.subdir;
...
function handleExpected(req, expected) {
	...
	for (var name in expected) {
		...
		log.verbose('expected request data validation [' + name + ']');
		if (error instanceof Error) {
			log.error('expected request data [' + name + '] validation failed (url:' + req.url + ')');
			return error;
		}
		log.verbose('expected request data validated [' + name + ']:', data);
	}
	return null;
}
