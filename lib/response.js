...
Response.prototype.setHeaders = function () {
	for (var name in this._resHeaders) {
		this._resource.rawResponse.setHeader(name, this._resHeaders[name]);
		log.verbose('setting response header:', name, this._resHeaders[name]);
	}
};
...
Response.prototype.download = function (content, fileName, status) {
	...
	this.header('Content-Type', mime.get(fileType) + '; charset=UTF-8');
...
Response.prototype.respond = function (headers, respondFunc, content, status) {
	...
	if (this._resource.responded) {
		...
		var dupResError = new Error('responded more than once: (url:' + rawReq.url + ') (request-id:' + rawReq.uniqueId + ')');
	...
	hook.exec(this, function () {
		...
		respondFunc(headers, that._resource.rawRequest, that._resource.rawResponse, content, status || that._defaultStatus);
...
function setupFinish(req, res, server, startTime) {
	...
	res.once('finish', function () {
		...
		var msg = 'request responded: (url:' + req.url + ') (request-id:' + req.uniqueId + ') (took:' + execTime + 'ms) (status:' + res.statusCode + ')';
...
function setupHeaders(res, status, headers, additionalHeaders) {
	for (var defName in defaultHeaders) {
		if (!headers.hasOwnProperty(defName)) {
			headers[defName] = defaultHeaders[defName];
		}
	}
