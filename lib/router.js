...
module.exports.setup = function (cb) {
	gracenode.lib.walkDir(config.controllerPath, function (error, list) {
		...
		for (var i = 0, len = list.length; i < len; i++) {
			...
			var paths = list[i].file.substring(0, list[i].file.lastIndexOf('.'));
...
module.exports.handle = function (url, res) {
	...
	if (config.urlPrefix) {
		...
		url = url.replace('/' + config.urlPrefix.replace(/^\/|\/$/g, '') + '/', '/');
	...
	if (!controllerMap[parsedUrl.controller]) {
		...
		parsedUrl.notFound = new Error(
			'controller method ' + parsedUrl.controller + '/' + parsedUrl.method + ' not found'
		);
	}
