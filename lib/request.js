...
function Request(request, response, resource) {
	// private
	this._props = {};
	...
	this.parameters = resource.params;
...
Request.prototype.setup = function (cb) {
	...
	this.extractQueries(this._request, function (error, dataHandler) {
		...
		logger.info('request:', that._method, that.url, '(request-id:' + that._request.uniqueId +')', that._dataHandler.getAll());
